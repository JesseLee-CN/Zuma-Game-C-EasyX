...
$$score = N^2 \times 10 \times \bigl(1 + (k-1) \times 0.5\bigr)$$

- `N` 为本轮消除球数（≥3），`k` 为连锁轮次（第 1 轮 ×1.0，第 2 轮 ×1.5，第 3 轮 ×2.0，依此类推）
- 一次性消除越多球得分越高（N² 增长），分次消除总分较低
- 右上角实时显示总分和剩余发射次数
- 消除时弹出浮动 "+分数" 动画
...
- 发射耗尽（GAME OVER）：同样淡出动画，按剩余球数 **×20** 扣分（下限 0），红色 "GAME OVER" 结算画面
